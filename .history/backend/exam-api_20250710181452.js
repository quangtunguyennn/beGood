...
app.post('/api/exams', (req, res) => {
  const { title, subject, date, time, done } = req.body;
  if (!title || !subject || !date || !time) return res.status(400).json({ error: 'Missing fields' });
  const exam = { title, subject, date, time, done: !!done };
